  ...
  if(mysqli_connect_error())
  {
    die ("Database Connection Error");
       ...
       $id=htmlspecialchars($_GET["id"]);
        $query = "SELECT * FROM `list` WHERE completed=1  AND team_went=".$id." ORDER BY `points`";

  $result = mysqli_query($link, $query);

    $delimiter = ",";
    if($id=='0')$team="Mighty Wizard";
                                if($id='1')$team="Invincible Warrior";
                                    if($id=='2')$team="Fiery Falcon";
                                    if($id=='3')$team="Super Spartan";
                                        if($id=='4')$team="Deadly Dragon";
                                        if($id=='5')$team="Storm Riders";    
    $filename = "npl-result.".$team.".csv";
